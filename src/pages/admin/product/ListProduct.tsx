import { CloseSquareFilled, ContainerOutlined, EditFilled, PlusCircleOutlined } from '@ant-design/icons'
              ...
              productList.map((item, index) => {
                return (
                  <tr className='border-b bg-white dark:border-gray-700 dark:bg-gray-800'>
                    ...
                    <td className='px-6 py-4'>{item.gender === 0 ? 'Male' : item.gender === 1 ? 'Female' : 'Both'}</td>
                    {/*  <td className='px-6 py-4'>
                      <SelectStatus status={item.status} id={item.id} onChange={handleEditStatus} />
                    </td> */}
                    <td className='px-6 py-4 text-right'>
                      <button onClick={(e) => history(`/admin/product/${item.productGroupId}`)}>
